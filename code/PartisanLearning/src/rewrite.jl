...
sample_overlapping_1d_to_2d_hack(d) = (sample_overlapping_normal(d), oneDBound[2])


#=
Thus, I have three general sampling functions: 
- `sample_uniform_pos` 
- `sample_simple_normal`
- `sample_overlapping_normal`

They work for ANY nissues. 
`sample_overlapping_normal` has a one parameter version that is its 1d version.

Finally, there is a hacky function only for visualization of the 1d model:
`sample_overlapping_1d_to_2d_hack`
 =#
...
function Voter(id::Int; nissues =  1,
    ...
    amIaCandidate = false
    myPartyId = -3
    v = Voter{nissues}(id, pos,    amIaCandidate,    myPartyId,    κ)
    return()
end
...
# this works for any nymber of dimensions
function sample_parties_ids(nparties::Int, model)
    ids = collect(abm.allids(model))
